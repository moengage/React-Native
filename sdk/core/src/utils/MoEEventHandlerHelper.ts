...
import {AUTHENTICATION_ERROR, PERMISSION_RESULT} from "..";
import {getAuthenticationErrorData, getPermissionResult} from "../moeParser/MoEngagePayloadParser";
    ...
    if (handler && type) {
        ...
        if (isValidObject(notificationPayload)) {
            if (type == "pushTokenGenerated") {
                ...
                const authError = getAuthenticationErrorData(notification);
                authError != undefined ? handler(authError) : null;
            } else {
