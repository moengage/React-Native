...
// using ReactNativeMoEngagePersonalize.podspec — this file is only consumed by
// React Native's SPM autolinker.
//
// The autolinker references this package through a symlink at
// <app>/build/generated/autolinking/libs/ReactNativeMoEngagePersonalize, so
// the relative `.package(path:)` entries below resolve against that location:
...
// look it up as "ReactNativeMoEngagePersonalize" rather than the name it would otherwise derive.

import PackageDescription

let package = Package(
    name: "ReactNativeMoEngagePersonalize",
    ...
    platforms: [.iOS(.v15)],
    products: [
        .library(name: "ReactNativeMoEngagePersonalize", targets: ["ReactNativeMoEngagePersonalize"]),
    ],
    dependencies: [
